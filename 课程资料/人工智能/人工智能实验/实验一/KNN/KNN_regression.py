...
def read_file(file_name,usecols=None):
    ...
    :return:返回读取的csv文件
    '''
    df = pd.read_csv('../data/lab1_data/regression_dataset/'+file_name,usecols=usecols)
    return df
...
def TFIDF(df,word_list):
    ...
    :return: 返回TFIDF矩阵
    '''
    df = read_file("regression_simple_test.csv",[0])
...
def KNN_predict(k,word_list,train_met,test_met,train_df):
    ...
    for i,record in enumerate(test_met):
        ...
        for i,train_record in enumerate(train_met):
            dis = cal_distance(record,train_record,n=2,cos=False)
...
def test_set_predict(k,res_filename,word_list,met,train_df):
    ...
    :return:
    '''
    predict_tag= KNN_predict(k, word_list, met[:6],met[-1:], train_df)
    print(predict_tag)
    csvFile = open(res_filename, "w")
    writer = csv.writer(csvFile)
    # 先写入columns_name
    writer.writerow(["textid", "anger", "disgust","fear","joy","sad","surprise"])  # 写入列的名称
    # 写入多行用writerows
    for i in range(len(predict_tag)):
        res = [i+1]+predict_tag[i].tolist()
        writer.writerow(res)
    csvFile.close()
...
def main():
    ...
    test_set_predict(3,"17341190_KNN_regression.csv",word_list,met,train_df)
    # x=[]
    # y=[]
    # validation_df = read_file('validation_set.csv')
    # for k in range(9,10):
    #     predict_tag,validation_df = KNN_predict(k,word_list,validation_df,met,train_df)
    #     res=cal_accuracy(predict_tag)
    #     # x.append(k)
    #     # y.append(res)
    #     print("KNN模型在k=%d回归问题的验证集上的相关度为：%s"%(k,res))
    # plt.plot(x, y)
    # plt.show()
